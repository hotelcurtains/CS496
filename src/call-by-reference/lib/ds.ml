...
(*  | ExtendEnvRec(v,param,body,tail) ->
    if id=v
    then Ok (RefVal (Store.new_ref st (ProcVal(param,body,env))))
    else apply_env st id tail
  *)                                    
...
let rec string_of_expval = function
  ...
  | ThunkVal (body,env) -> "ThunkVal ("^string_of_expr body^","^ string_of_env' env^")"
and
